    ...
    async newindc(@Res() res, @Req() req,  @Body('indc') ind: any ){
        ...
        try {
            ...
            const newInd = await this.prisma.indicateur.create({data:{
                ...
                Action: ind.action,
                responsableId: ind.responsable,
                delaiTrait: ind.detail,
                suivi: ind.suivi,
                actionEfficace: ind.acteffi,
                criteresDeval: ind.criter,
                dateEvaluation: ind.dateval,
                observation: ind.observ
