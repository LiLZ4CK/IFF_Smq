	...
	constructor(private readonly adminService: AuditsService,
                ...
                async getall(@Res() res: Response) {
                  try {
                    ...
                    const auditsToUpdate2 = audits.filter(audit => 
                      audit.status === 1 && new Date(audit.realisationDate) > now
                    );
                    ...
                    const updatePromises2 = auditsToUpdate2.map(audit =>
                      this.prisma.audit.update({
                        where: { id: audit.id },
                        data: { status: 0 }
                      })
            ...
            async createNew(@Res() res, @Req() req) {
              ...
              upload(req, res, async (err: any) => {
                ...
                try {
                  ...
                  // Fetch the existing audit to retain the old responsableId if not provided
                  const existingAudit = await this.prisma.audit.findUnique({ where: { id } });
                  if (!existingAudit) {
                    return res.status(404).send('Audit not found');
                  }
            
                  const responsableId = body.responsableId ? parseInt(body.responsableId) : existingAudit.responsableId;
            
                  if (isNaN(responsableId)) {
                    throw new Error('Invalid responsableId');
                  }
            
                  console.log('Using responsableId:', responsableId);
                  ...
                  await this.prisma.audit.update({
                    ...
                    data: {
                      ...
                      lieu: body.lieu,
                      path: filePath, // Assuming filePath is defined somewhere
                    }
                  });
            
                  return res.status(200).send('Audit updated successfully');
                } catch (error) {
                  console.log(error);
                  return res.status(500).send('Internal server error');
                }
