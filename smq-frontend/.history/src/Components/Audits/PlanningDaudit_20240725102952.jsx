  ...
  const Events = async (args) => {
    try {
      ...
      if (args.requestType === 'eventChanged') {
        ...
        try {
          const response = await fetch('http://localhost:3000/audits/edit2', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
        ...
        const data = {
          id: args.data[0].id,
          responsableId: document.querySelector('select[name="Responsable"]').value,
          type: document.querySelector('input[name="Type"]').value,
          processus: document.querySelector('select[name="Processus"]').value,
          planningDate: document.querySelector('input[name="StartTime"]').value,
          realisationDate: document.querySelector('input[name="EndTime"]').value,
          lieu: 'IFF'
        };
