...
const IndicateurList = () => {
  ...
  const [filteredIndicateur, setFilteredActions] = useState([]);
  const [filterValue, setFilterValue] = useState('');

  const filterActions = (Actions, filterValue) => {
    if (!filterValue) return Actions; // No filter, return all Actions

    return Actions.filter(action =>
      action.type.toLowerCase().includes(filterValue.toLowerCase()) ||
      action.processus.toLowerCase().includes(filterValue.toLowerCase()) ||
      action.code.toLowerCase().includes(filterValue.toLowerCase())
    );
  };
